...
AREA_MAX        <- 450    # ceiling to exclude clumps
INTENSITY_MIN   <- 300    # DAPI intensity floor
CIRCULARITY_MIN <- 0.08   # Sphericity doublet / fragment cutoff
...
process_plate_files <- function(nucleus_file, virus_file) {
  ...
  sphericity_col <- names(df_nuclei)[grepl("Sphericity", names(df_nuclei), ignore.case = TRUE)][1]
  ...
  if (!is.na(sphericity_col) && sphericity_col %in% names(df_nuclei)) {
    nuclei_clean$Sphericity <- df_nuclei[[sphericity_col]]
  } else {
    nuclei_clean$Sphericity <- 1.0
  }
...
# --- Step 1: Soft Pre-Gating to Protect Pyknotic Cells -----------------------
master_df_pre <- master_df_annotated %>%
  dplyr::filter(Area > AREA_MIN & dapi_intensity > INTENSITY_MIN)

# --- Step 2: Density-Based Sphericity Truncation ----------------------------
...
p_doublets <- ggplot(master_df_pre, aes(x = Area, y = Sphericity)) +
  ...
  labs(
    ...
    subtitle = "Single cells sit above Sphericity cut (dashed line)",
    x = "Nuclear Volume / Area (pixels²)", y = "Sphericity"
  ) +
  theme_bw(base_size = 8) +
  theme(legend.position = "none")

# --- Step 3: Statistical Cell Cycle Gating ----------------------------------
master_df_singlets <- master_df_pre %>%
  dplyr::filter(Is_Singlet)
...
p_cycle <- ggplot(master_df_singlets, aes(x = Area, y = integrated_intensity)) +
  geom_bin2d(bins = 200) +
  scale_fill_gradientn(colors = c("navy", "deepskyblue", "green", "yellow", "red"), trans = "log10", name = "Cells") +
  stat_ellipse(type = "norm", level = 0.95, color = "black", linetype = "dashed", linewidth = 1) +
  scale_y_continuous(labels = scales::label_scientific()) +
  labs(
    title = "Cell Cycle & Quality Control",
    subtitle = "Healthy interphase & mitotic cells contained in 95% DNA Ellipse",
...
ggsave("Plots/nuclear_gating_qc.tiff", qc_plates_plot, width = 9, height = 4, units = "in", dpi = 300, compression = "lzw", device = ragg::agg_tiff)
...
p_gmm_density <- ggplot(ctrl_dens_df, aes(x = x_norm, fill = Group)) +
  ...
  annotate("label", x = threshold_final, y = 1.1,
           label = paste0("GMM Infection Gate = ", round(threshold_final, 3)),
           fill = "firebrick3", color = "white", fontface = "bold", size = 3, label.size = 0) +
...
print(p_gmm_density)
ggsave("Plots/infection_gate_gmm_density.tiff", p_gmm_density, width = 5.5, height = 3.8, units = "in", dpi = 300)
```

# Single-Cell Population Shift Across Doses (Ridgeline Density)

```{r ridgeline-plot, warning=FALSE, message=FALSE, fig.width=6, fig.height=5}
ridge_df <- master_df_final %>%
  filter(Treatment %in% c("Mock", "Virus Only", "Virus + Compound")) %>%
  mutate(Condition = case_when(
    Treatment %in% c("Mock", "Mock+Ab") ~ "Mock",
    Treatment == "Virus Only"           ~ "Virus Only (0 µM)",
    TRUE                                ~ paste0(Concentration_uM, " ", DOSE_UNIT)
  ))

dose_order <- c(
  "Mock", "Virus Only (0 µM)",
  paste0(sort(unique(master_df_final$Concentration_uM[!is.na(master_df_final$Concentration_uM) & master_df_final$Concentration_uM > 0])), " ", DOSE_UNIT)
)
ridge_df <- ridge_df %>%
  mutate(Condition = factor(Condition, levels = rev(dose_order)))

p_ridgeline <- ggplot(ridge_df, aes(x = x_norm, y = Condition, fill = Condition)) +
  ggridges::geom_density_ridges(alpha = 0.7, scale = 1.3, color = "white", linewidth = 0.3) +
  geom_vline(xintercept = threshold_final, linetype = "dashed", color = "firebrick3", linewidth = 0.7) +
  labs(
    title = "Single-Cell Viral Reporter Shift Across Compound Doses",
    subtitle = "Dashed line indicates 2-GMM population infection threshold",
    x = "Normalized Virus Intensity",
    y = "Treatment / Compound Dose"
  ) +
  theme_bw(base_size = 8) +
  theme(legend.position = "none", plot.title = element_text(face = "bold"))

print(p_ridgeline)
ggsave("Plots/single_cell_ridgeline_shift.tiff", p_ridgeline, width = 5.5, height = 4.8, units = "in", dpi = 300)
```

# Host Cell Viability & Cytotoxicity (CC50)
...
print(p_toxicity)
ggsave("Plots/host_cell_viability_cc50.tiff", p_toxicity, width = 5.5, height = 3.8, units = "in", dpi = 300)
```
...
if (!is.null(fit_rel)) {
  ...
  p_dr_rel <- p_dr_rel +
    ...
    annotate("label", x = ec50_rel, y = 25, label = ec50_rel_lab,
             fill = "#2B5C8F", color = "white", fontface = "bold", size = 3, label.size = 0)
}
...
ggsave("Plots/compound_dose_response_relative.tiff", p_dr_rel,
       width = 5.5, height = 4.0, units = "in", dpi = 300)
```
