...
process_plate_files <- function(nucleus_file, virus_file) {
  ...
  if (!"Sphericity" %in% names(df_nuclei)) {
...
# Cell Gating (QC Scatterplot)

```{r gating-plot, warning=FALSE, message=FALSE, fig.width=6, fig.height=4}
master_df_pre <- master_df_annotated %>%
  dplyr::mutate(
    Pass_QC = Area >= AREA_MIN & Area <= AREA_MAX & dapi_intensity >= INTENSITY_MIN & Sphericity >= CIRCULARITY_MIN,
    integrated_intensity = Area * dapi_intensity
  )

# Display QC Gating Scatterplot
p_qc_gating <- ggplot(master_df_pre, aes(x = Area, y = dapi_intensity, color = Pass_QC)) +
  geom_point(alpha = 0.35, size = 0.9) +
  geom_vline(xintercept = c(AREA_MIN, AREA_MAX), linetype = "dashed", color = "firebrick", linewidth = 0.6) +
  geom_hline(yintercept = INTENSITY_MIN, linetype = "dashed", color = "firebrick", linewidth = 0.6) +
  scale_color_manual(
    values = c("TRUE" = "#2B5C8F", "FALSE" = "#D95F02"),
    labels = c("FALSE" = "Excluded Debris / Clump", "TRUE" = "Passed Gating (Singlet)")
  ) +
  labs(
    title = "Single-Cell Quality Control Gating",
    subtitle = paste0("Limits: Area [", AREA_MIN, "–", AREA_MAX, " px²], DAPI > ", INTENSITY_MIN, " a.u."),
    x = "Nuclear Area (pixels²)",
    y = "Mean DAPI Intensity (a.u.)",
    color = "Cell Status"
  ) +
...
print(p_qc_gating)
ggsave("Plots/qc_gating_scatterplot.tiff", p_qc_gating, width = 5, height = 3.8, units = "in", dpi = 300)

master_df_gated <- master_df_pre %>% dplyr::filter(Pass_QC)
```
...
p_gmm_density <- ggplot(ctrl_dens_df, aes(x = x_norm, fill = Group)) +
  ...
  scale_fill_manual(values = c("Mock (Negative Control)" = "grey60", "Virus Only (Positive Control)" = "#D95F02")) +
  labs(
