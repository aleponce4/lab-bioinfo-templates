...
sample_data %>%
  ...
  knitr::kable(
    ...
    col.names = c("Sample ID", "Longitude", "Latitude", "Clade", "Year", "Host")
  )
...
data.frame(
  ...
  Length_bp   = seq_widths,
  Ambig_Bases = ambig_counts
) %>%
  arrange(desc(Ambig_Bases)) %>%
  head(10) %>%
  knitr::kable(
    caption   = "Sequence Alignment Quality & Ambiguous Base Count",
    col.names = c("Sequence Header", "Length (bp)", "Ambiguous Bases (N)")
  )
```
