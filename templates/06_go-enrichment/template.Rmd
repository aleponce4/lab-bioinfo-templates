...
# Bubble Plots (Gene Set Visualization) {.tabset .tabset-fade}

```{r bubble-generate, fig.height=8, fig.width=8, message=FALSE, warning=FALSE, results='asis', echo=FALSE}
...
```{r go-plots-tabset, fig.width=8, fig.height=6, message=FALSE, warning=FALSE, results='asis', echo=FALSE}
...
# KEGG Pathway Enrichment {.tabset .tabset-fade}
...
```{r kegg-plots-tabset, fig.width=8, fig.height=6, message=FALSE, warning=FALSE, results='asis', echo=FALSE}
for (comp in names(kegg_results)) {
  kk <- kegg_results[[comp]]$kk
  if (is.null(kk) || nrow(as.data.frame(kk)) == 0) next
