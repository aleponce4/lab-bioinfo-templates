...
# Volcano axis limits. Points beyond them are squished onto the boundary, never
# dropped - a volcano that silently deletes its most significant hits is worse
# than one with a visible pile-up at the edge.
VOLCANO_XLIM <- c(-6, 6)
VOLCANO_YLIM <- 12
...
VIRAL_FEATURE_CONFIG <- list(
  ...
  feature_patterns = c(
    ...
    Pathogen_gRNA = "^Pathogen_gRNA$",
    Pathogen_sgRNA = "^Pathogen_sgRNA$"
  ),
  feature_lengths_bp = c(
    Pathogen_genome = 11500,
    Pathogen_gRNA = 11500,
    Pathogen_sgRNA = 3900
  ),
  # Added to every value before the log10 axis transform, so zero-count samples
  # stay on the plot without being reported as a measured background level.
  log_pseudocount = 0.5,
  # position_jitter() is random. Without a fixed seed every render moves the
  # points and rewrites the SVG, which is permanent churn in version control.
  jitter_seed = 42
)
## -----------------------------------------------------------------------------
```
...
for (i in seq_len(nrow(contrast_table))) {
  ...
  contrast_spec <- c(row_i$factor, row_i$numerator, row_i$denominator)
  ...
  shrunk_obj <- shrink_log2_fold_changes(fit_i$dds, contrast_spec, result_obj)

  results_list[[comparison_label]] <- extract_results_tidy(
    result_obj = result_obj,
    shrunk_obj = shrunk_obj,
    comparison_label = comparison_label,
    model_id = row_i$model_id,
    contrast_name = row_i$contrast_name
  )
...
pca_data_list <- lapply(names(model_fits), function(model_id) {
  ...
  # plotPCA() returns the variance explained only as an attribute, and bind_rows()
  # drops attributes. Promote it to real columns (and into the facet label) so the
  # published PCA does not ship with bare "PC1"/"PC2" axes.
  percent_var <- attr(pca_tbl, "percentVar")
  pc1_pct <- round(100 * percent_var[1])
  pc2_pct <- round(100 * percent_var[2])

  pca_tbl %>%
    mutate(
      model_id = model_id,
      pc1_percent_var = pc1_pct,
      pc2_percent_var = pc2_pct,
      model_label = sprintf("%s  (PC1 %d%%, PC2 %d%%)", model_id, pc1_pct, pc2_pct)
    )
})
...
p_pca <- ggplot(pca_data, aes(x = PC1, y = PC2, color = .data[[PCA_COLOR_COLUMN]])) +
  ...
  labs(
    ...
    x = "PC1 (% variance explained shown per panel)",
    y = "PC2 (% variance explained shown per panel)",
...
volcano_plots <- lapply(comparison_levels, function(label) {
  ...
  # Count what the axis limits will clamp so the figure can say so out loud.
  n_squished <- sum(
    df$log2FoldChange < VOLCANO_XLIM[1] | df$log2FoldChange > VOLCANO_XLIM[2] |
      df$log_padj > VOLCANO_YLIM,
    na.rm = TRUE
  )
  ...
  ggplot(df, aes(x = log2FoldChange, y = -log10(padj_plot), color = direction)) +
    ...
    # oob = squish clamps out-of-range points to the axis boundary. A plain
    # `limits =` would DELETE them, and the deleted points are precisely the
    # strongest hits (padj underflows to ~1e-308, i.e. -log10 ~ 308).
    scale_x_continuous(limits = VOLCANO_XLIM, oob = scales::squish) +
    scale_y_continuous(limits = c(0, VOLCANO_YLIM), oob = scales::squish) +
    labs(
      title = label,
      x = expression(log[2] * " Fold Change"),
      y = "-log10(FDR)",
      caption = if (n_squished > 0) {
        sprintf("%d point(s) beyond the axis limits are drawn clamped at the boundary.", n_squished)
      } else {
        NULL
      }
    ) +
...
if (isTRUE(OPTIONAL_MODULES$ma_plots)) {
  ma_plots <- lapply(comparison_levels, function(label) {
    ...
    # MA plots are the canonical place for shrunken fold changes: without them the
    # low-count end of the x axis is dominated by high-variance noise.
    ggplot(df, aes(x = baseMean_plot, y = log2FoldChange_shrunk, color = direction)) +
      geom_point(alpha = 0.55, size = 0.8) +
      scale_x_log10(labels = scales::comma) +
      scale_color_manual(values = c(Up = "#a00000", Down = "#2066a8", NS = "gray70")) +
      geom_hline(yintercept = c(-LFC_THRESHOLD, LFC_THRESHOLD), linetype = "dashed", color = "darkgrey") +
      labs(title = label, x = "Base mean (log10 scale)",
           y = expression("Shrunken " * log[2] * " Fold Change")) +
      theme_bw(base_size = 10, base_family = "sans") +
      theme(
        plot.title = element_text(face = "bold", hjust = 0.5),
        legend.position = "none",
...
if (isTRUE(OPTIONAL_MODULES$top_heatmaps)) {
  for (label in comparison_levels) {
    # Only genes that actually pass FDR + |LFC| belong under a "Top DEGs" heading.
    # Sorting by padj alone would happily show 25 rows when nothing is significant.
    ...
    if (length(top_genes) < 2) {
      message("Skipping top-DEG heatmap for '", label, "': ",
              nrow(result_df), " gene(s) pass FDR < ", FDR_THRESHOLD,
...
```{r bubble-carousel-data, fig.height=9, fig.width=8, message=FALSE, warning=FALSE, echo=FALSE}
bubble_names <- names(GENE_SETS)
for (pathway_name in bubble_names) {
  ...
  stub <- file.path("Bubble_Plots", plot_file_stub(paste0("bubble_", pathway_name)))
  ggsave(file.path(OUTPUT_DIR, paste0(stub, "_carousel.png")),
         plot = p, width = 8, height = 9, dpi = 150)
}
```

```{r bubble-carousel-html, echo=FALSE, results='asis'}
if (length(bubble_names) > 1) {
  ...
  for (i in seq_along(bubble_names)) {
    ...
    stub <- file.path("Bubble_Plots", plot_file_stub(paste0("bubble_", nm)))
    img_src <- file.path(OUTPUT_DIR, paste0(stub, "_carousel.png"))
    html <- c(html, sprintf('    <div class="carousel-item%s">', active),
              sprintf('      <img src="%s" class="d-block w-100" alt="%s" style="background:white;border-radius:4px;">', img_src, nm),
              '    </div>')
  }
  html <- c(html, '  </div>',
...
} else if (length(bubble_names) == 1) {
  stub <- file.path("Bubble_Plots", plot_file_stub(paste0("bubble_", bubble_names[1])))
  cat(sprintf('<img src="%s/%s_carousel.png" class="d-block w-100" style="background:white;border-radius:4px;">\n', OUTPUT_DIR, stub))
}
```

# Triangle Plots

```{r triangle-carousel-data, fig.height=9, fig.width=6, message=FALSE, warning=FALSE, echo=FALSE}
triangle_names <- names(GENE_SETS)
for (pathway_name in triangle_names) {
  ...
  stub <- file.path("Triangle", plot_file_stub(paste0("triangle_", pathway_name)))
  ggsave(file.path(OUTPUT_DIR, paste0(stub, "_carousel.png")),
         plot = p, width = 6, height = 9, dpi = 150)
}
```

```{r triangle-carousel-html, echo=FALSE, results='asis'}
if (length(triangle_names) > 1) {
  html <- c('<div id="tri-carousel" class="carousel slide" data-bs-ride="false" style="background:#f8f8f8;border-radius:6px;padding:10px;">',
  '  <div class="carousel-indicators">')
  for (i in seq_along(triangle_names)) {
    active <- if(i == 1) ' class="active" aria-current="true"' else ''
    html <- c(html, sprintf('    <button type="button" data-bs-target="#tri-carousel" data-bs-slide-to="%d"%s></button>', i-1, active))
  }
  html <- c(html, '  </div>', '  <div class="carousel-inner">')
  for (i in seq_along(triangle_names)) {
    nm <- triangle_names[i]
    active <- if(i == 1) ' active' else ''
    stub <- file.path("Triangle", plot_file_stub(paste0("triangle_", nm)))
    img_src <- file.path(OUTPUT_DIR, paste0(stub, "_carousel.png"))
    html <- c(html, sprintf('    <div class="carousel-item%s">', active),
              sprintf('      <img src="%s" class="d-block w-100" alt="%s" style="background:white;border-radius:4px;">', img_src, nm),
              '    </div>')
  ...
  html <- c(html, '  </div>',
  '  <button class="carousel-control-prev" type="button" data-bs-target="#tri-carousel" data-bs-slide="prev"><span class="carousel-control-prev-icon" aria-hidden="true"></span></button>',
  '  <button class="carousel-control-next" type="button" data-bs-target="#tri-carousel" data-bs-slide="next"><span class="carousel-control-next-icon" aria-hidden="true"></span></button>',
  '</div>')
  knitr::asis_output(paste(html, collapse = "\n"))
} else if (length(triangle_names) == 1) {
  stub <- file.path("Triangle", plot_file_stub(paste0("triangle_", triangle_names[1])))
  cat(sprintf('<img src="%s/%s_carousel.png" class="d-block w-100" style="background:white;border-radius:4px;">\n', OUTPUT_DIR, stub))
}
```
...
if (isTRUE(OPTIONAL_MODULES$viral_features)) {
  ...
  # Host size factors come from the full host count matrix, so this is a genuine
  # library-depth correction and is directly comparable between samples.
  ...
  # Same values, additionally divided by feature length, so the three viral
  # features are comparable with each other (see normalize_by_length_kb).
  viral_sf_norm_per_kb <- normalize_by_length_kb(viral_sf_norm, viral_lengths_bp)
  ...
  viral_long <- bind_rows(
    ...
    as.data.frame(as.table(viral_sf_norm_per_kb), stringsAsFactors = FALSE) %>%
      transmute(feature = Var1, sample_name = Var2, value = Freq, metric = "Normalized counts per kb")
  ) %>%
    left_join(
      metadata %>%
        tibble::rownames_to_column("sample_name"),
    ...
    mutate(
      # A real pseudocount, not pmax(): flooring every zero-viral sample at the
      # same value fabricates a flat plateau that reads as measured background.
      value_plot = value + VIRAL_FEATURE_CONFIG$log_pseudocount,
  ...
  p_viral <- ggplot(
    ...
    geom_point(
      position = position_jitter(width = 0.08, height = 0,
                                 seed = VIRAL_FEATURE_CONFIG$jitter_seed),
      alpha = 0.5, size = 1.7
    ) +
    ...
    labs(
      ...
      x = group_col,
      y = sprintf("Abundance + %g (log10 scale)", VIRAL_FEATURE_CONFIG$log_pseudocount),
      color = group_col
    ) +
    theme_bw(base_size = 10, base_family = "sans") +
    theme(
  ...
  viral_tables <- list(
    ...
    viral_sf_norm_per_kb = viral_sf_norm_per_kb,
...
safe_names  <- vapply(comparison_levels, sanitize_filename, character(1))
sheet_names <- make_unique_sheet_names(safe_names)

for (i in seq_along(comparison_levels)) {
  label <- comparison_levels[[i]]
  export_df <- results_list[[label]] %>%
    arrange(padj, desc(abs(log2FoldChange_shrunk)))

  write_csv(export_df, file.path(SUMMARY_DIR, paste0("DEGs_", safe_names[[i]], ".csv")))
  deg_workbook[[sheet_names[[i]]]] <- export_df
}
