...
tf_activity %>% group_by(Comparison) %>% slice_max(abs(activity_z), n = 5) %>% ungroup()
```
...
plot_network <- function(edges, nodes, title) {
  ...
  edge_xy <- edges %>% left_join(coords %>% select(node, x, y), by = c("source" = "node")) %>% rename(x = x, y = y) %>% left_join(coords %>% select(node, x, y), by = c("target" = "node")) %>% rename(xend = x, yend = y)
  ggplot() +
    geom_segment(data = edge_xy, aes(x, y, xend = xend, yend = yend, color = edge_color(sign)), linewidth = 0.7,
...
write_csv(module_deg_summary, file.path(paths$tables, "module_significant_deg_summary.csv"))
module_deg_summary
```
